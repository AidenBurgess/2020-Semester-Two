...
void quick_sort(struct block my_data)
{
    ...
    { // the parent
        quick_sort_left(left_side);
        // wait(NULL);
        read(my_pipe[0], right_side.data, sizeof(int) * right_side.size);
        // print_data(right_side);
    }
    else
    { // the child
        quick_sort_right(&right_side);
        // puts("Finished sorting in child");
        // print_data(right_side);
        write(my_pipe[1], right_side.data, sizeof(int) * right_side.size);
        exit(0);
    }
}
